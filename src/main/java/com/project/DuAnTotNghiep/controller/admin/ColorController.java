...
public class ColorController {
    ...
    public String addColor(RedirectAttributes redirectAttributes, @Validated @ModelAttribute("Color") Color color) {
        try {
            ...
            redirectAttributes.addFlashAttribute("successMessage", "Thêm % đường mới thành công");
        }
        catch (Exception e) {
    ...
    public String update(@PathVariable("id") Long id,
        ...
        if (colorService.existsById(id)) {
            try {
                ...
                redirectAttributes.addFlashAttribute("successMessage", "Mã % đường " + color1.getCode() + " cập nhật thành công");
